...
import { ClienteFotoView, GRFoto } from '../../../../../services/contrato/clienteFoto/clienteFoto';
...
export class FotoClienteComponent implements OnInit, OnDestroy {
  ...
  // Array Lista
  public dado: GRFoto = new GRFoto();
  public listaDados: ClienteFotoView[] = new Array();
  public listaDadosOld: ClienteFotoView[] = new Array();
  ...
  gerarBackup(): void {
    ...
    this.clienteFotoService.gerarBackup().then(relatorio => {
      ...
      const nomeArquivo = "backuo_clienteFoto_" + formattedDate + ".zip"
  ...
  isVerificaProcessamento(id: any): Promise<any> {
    return this.clienteFotoService.isVerificaProcessamento(id).toPromise();
  }

  gerar(tipoGeracao: any, id: any): Promise<any> {
    return this.clienteFotoService.gerar(tipoGeracao, id).toPromise();
  }
